...
public class CertTools {
  ...
  // Distinguished Name (DN) is created by concatinating each entry in the tree, starting from the root and going down to the named entry. Example: cn=John Doe, ou=Sales, dc=example, dc=com.
  // objectClass: Defines the type of object an entry represents (e.g., "person", "organization", "alias").
  // aliasedObjectName: Used when an entry is an alias, pointing to another entry.
  // cn (Common Name): Typically a person's full name or the name of an object. Like "John Doe" or "www.google.com" or "*.eclipse.com"
  // sn (Surname): A person's surname.
  // givenName: A person's given name.
  // ou (Organizational Unit): Represents a unit within an organization.
  // o (Organization): Represents the name of an organization. (Such as company name)
  // l (Locality): Represents a locality or city.
  // st (State): Represents a state or province.
  // c (Country): Represents a country.
  ...
  // proxyAddresses: A list of email addresses associated with an object.
  // supportedAlgorithms: Defines the algorithms supported by the directory service.
  // deltaRevocationList: Used for delta CRLs (Certificate Revocation Lists).
  // dmdName: Used in the X.500 directory model.
  // legacyExchangeDN: An X.500 address used in Exchange environments, especially for mailboxes and mail-enabled contacts.
  // userPrincipalName: The user's login name (e.g., username@domain.com).
  // sAMAccountName: The pre-Windows 2000 login name (e.g., username).
  // objectGUID: A unique identifier for an object in Active Directory.
  // userAccountControl: Flags that control the behavior of a user account (e.g., disabled, password not required).
  // passwordLastSet: Timestamp of the last password change.
  // lastLogon: Timestamp of the last logon.
  // memberOf: Indicates the groups a user or object is a member of.
  // member: Indicates the members of a group.
  ...
  static {
    ...
    // Oranization like name of company
    ...
    // Required fqdn or wildcarded fqdn
